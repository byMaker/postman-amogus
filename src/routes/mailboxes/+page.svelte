			...
			<thead class="text-xs font-black uppercase tracking-widest text-amogus-blue hidden md:table-header-group">
				<tr>
					<th class="px-6 py-5 cursor-pointer hover:text-amogus-blue select-none transition-colors" onclick={() => toggleSort('localPart')}>
						{t('mailbox.table.email')} <span class="text-amogus-blue ml-1 font-bold">{sortField === 'localPart' ? (sortDir === 'asc' ? '↑' : '↓') : ''}</span>
					</th>
					<th class="px-6 py-5 cursor-pointer hover:text-amogus-blue select-none transition-colors" onclick={() => toggleSort('fullName')}>
						{t('mailbox.table.fullname')} <span class="text-amogus-blue ml-1 font-bold">{sortField === 'fullName' ? (sortDir === 'asc' ? '↑' : '↓') : ''}</span>
					</th>
					...
					<th class="px-6 py-5 cursor-pointer hover:text-amogus-blue select-none transition-colors" onclick={() => toggleSort('quotaMb')}>
						{t('mailbox.table.quota')} <span class="text-amogus-blue ml-1 font-bold">{sortField === 'quotaMb' ? (sortDir === 'asc' ? '↑' : '↓') : ''}</span>
					</th>
			...
			<tbody class="divide-y divide-slate-100 bg-white block md:table-row-group md:rounded-b-[32px]">
				{#each filteredUsers as user, index}
					...
					<tr class="hover:bg-slate-50 transition-colors block md:table-row p-4 md:p-0 border-b border-slate-100 last:border-0 md:border-none group">
						<td class="flex justify-between items-center md:table-cell px-2 py-3 md:px-6 md:py-5 border-b border-slate-50 md:border-none {index === filteredUsers.length - 1 ? 'md:rounded-bl-[32px]' : ''}">
							<span class="md:hidden text-[10px] text-slate-400 font-bold uppercase tracking-widest">{t('mailbox.table.email')}</span>
						...
						<td class="flex justify-between items-center md:table-cell px-2 py-3 md:px-6 md:py-5 font-medium text-slate-600 border-b border-slate-50 md:border-none">
							<span class="md:hidden text-[10px] text-slate-400 font-bold uppercase tracking-widest">{t('mailbox.table.fullname')}</span>
						...
						<td class="flex justify-between items-center md:table-cell px-2 py-3 md:px-6 md:py-5 border-b border-slate-50 md:border-none">
							...
							<div class="flex flex-wrap gap-2 items-center">
								...
								{#if !!user.useForAliasesDomains}
									<span class="whitespace-nowrap rounded-full bg-indigo-100 px-3 py-1 text-xs font-bold text-indigo-700">{t('status.domain_alias')}</span>
								{/if}
							</div>
						</td>
						<td class="flex justify-between items-center md:table-cell px-2 py-3 md:px-6 md:py-5 md:w-48 border-b border-slate-50 md:border-none">
							<span class="md:hidden text-[10px] text-slate-400 font-bold uppercase tracking-widest">{t('mailbox.table.quota')}</span>
						...
						<td class="flex justify-between items-center md:table-cell px-2 py-3 md:px-6 md:py-5 md:text-right {index === filteredUsers.length - 1 ? 'md:rounded-br-[32px]' : ''}">
							...
							<div class="flex justify-end gap-3 transition-opacity">
								<Tooltip text={t('graph.title')} position="top">
									<button onclick={() => openGraphModal(user)} class="relative p-2 rounded-full transition-colors {totalRoutes > 0 ? 'text-amber-500 hover:text-white hover:bg-amber-500 bg-amber-50' : 'text-slate-400 hover:text-slate-600 hover:bg-slate-100 bg-slate-50'}" title={t('graph.title')}>
								...
								<Tooltip text={t('btn.edit')} position="top">
									<button onclick={() => openEditModal(user)} class="text-amogus-blue hover:text-white hover:bg-amogus-blue bg-blue-50 p-2 rounded-full transition-colors" title={t('btn.edit')}>
										<NotePencil size={24} weight="fill" />
									</button>
								</Tooltip>
								<Tooltip text={t('btn.delete')} position="top">
									<button onclick={() => requestDelete(user)} class="text-rose-600 hover:text-white hover:bg-rose-600 bg-rose-50 p-2 rounded-full transition-colors" title={t('btn.delete')}>
...
<Modal bind:show={showDeleteModal} title={t('modal.delete')}>
	<div class="bg-rose-50 border border-rose-200 text-rose-800 p-6 rounded-2xl mb-8 shadow-sm">
		...
		<p class="mb-5 text-sm">{t('modal.about_to_delete')} <span class="font-bold px-1.5 py-0.5 bg-rose-200 rounded text-rose-900">{userToDelete?.localPart}@{userToDelete?.domain}</span>.</p>
	</div>
	...
	<form method="POST" action={isEditMode ? "?/update" : "?/create"} use:enhance={() => {
		...
		{#if !isEditMode}
			<div class="form-control">
				<div class="label"><span class="label-text font-bold text-slate-700">{t('form.email')}</span></div>
		...
		{:else}
			...
			<div class="bg-blue-50 p-4 rounded-2xl border border-blue-100 mb-6">
				<p class="text-sm font-medium text-slate-500 mb-1">{t('form.editing_account')}</p>
				<p class="text-xl font-black text-amogus-dark">{currentUser.localPart}@{currentUser.domain}</p>
			</div>
		{/if}

		<div class="form-control">
			<div class="label"><span class="label-text font-bold text-slate-700">{t('form.fullname')} <span class="text-slate-400 font-normal">{t('form.optional')}</span></span></div>
			<input 
				type="text" 
				name="fullName" 
		...
		<div class="form-control">
			<div class="label">
				<span class="label-text font-bold text-slate-700">
					{t('form.password')}
					{#if isEditMode}
						<span class="text-slate-400 font-normal text-xs ml-2">{t('form.optional')}</span>
					{/if}
				</span>
			</div>
			<input 
		...
		<div class="flex flex-col sm:flex-row gap-4">
			<div class="form-control flex-1">
				<div class="label"><span class="label-text font-bold text-slate-700">{t('form.quota_mb')}</span></div>
				<input 
					type="number" 
					name="quotaMb" 
					...
					min="0"
				/>
				<div class="label"><span class="label-text-alt text-slate-500">{t('form.unlimited')}</span></div>
			</div>
			<div class="form-control flex-1">
				<div class="label"><span class="label-text font-bold text-slate-700">{t('form.quota_msg')}</span></div>
				<input 
					type="number" 
					name="quotaMessages" 
		...
		<div class="flex flex-col gap-3 pt-2">
			...
			<label class="cursor-pointer flex items-center gap-3 w-fit">
				...
				<span class="font-bold text-slate-700">{t('form.use_domain_aliases')}</span>
			</label>
		</div>
	...
	{#if graphUser}
		...
		{#if !isDomainActive}
			<div class="mb-6 mt-4 rounded-xl bg-rose-50 p-4 border border-rose-100 flex items-center gap-4 text-rose-700">
				...
				<div>
					<p class="font-bold text-base mb-0.5">{t('graph.domain_disabled')}</p>
					<p class="text-sm">{t('graph.domain_disabled.desc').replace('{domain}', graphUser.domain)}</p>
				</div>
			</div>
		{/if}
